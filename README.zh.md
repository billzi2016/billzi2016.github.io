...
- 小型 hotfix、单 commit 文档修正、README/AGENTS/spec 小补充、文案或样式微调，如果范围清楚、风险低、不会形成多轮提交，可以直接在 `main` 上完成。
- 如果预计会产生多个 commit，或涉及结构性改动、构建/部署逻辑、跨模块行为、复杂样式、运行时脚本、数据迁移、高风险区域，必须使用短生命周期 feature 分支。
...
- 小型 hotfix、单 commit 文档修正、README/AGENTS/spec 小补充、文案或样式微调，在范围清楚、风险低、不影响网站构建结果时可以直接进入 `main`。
- 如果一个任务预计需要多个 commit，或会改结构、构建/部署、跨模块行为、运行时脚本、数据迁移或高风险区域，必须使用短生命周期 feature 分支。
