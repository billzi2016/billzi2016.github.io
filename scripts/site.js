  ...
  page: {
    ...
    personal: {
      ...
      zh: {
        ...
        aiArchitectText:
          "我现在的一个长期爱好，是把自己训练成 AI 全栈架构师：借助 AI 辅助工程能力，通过 TDD、SDD 和验证驱动的工程流程，而不是 vibe coding 或凭感觉编程，产出工业级代码，并从产品逻辑、前端界面、后端服务、部署、可观测性、自动化到长期运维，完成整套互联网技术设施的设计、搭建、验证和维护。从 2025 年 5 月开始，我就以高强度方式使用 AI 工具，积累了充分的 AI-Augmented Engineering Scope 合作经验。很多 AI 辅助工程中常见的问题和失效模式，我都已经在实际协作中反复遇到过，因此我的重点不是简单接受生成代码，而是进行清晰规格定义、审查、调试、集成和验证。我的理解是，AI 辅助编程真正需要的是架构能力和持续不断的试错。只有持续、巨量地和 AI 一起工作，才能真正掌握 AI 工作的优势和问题，并及时调整自己适应 AI 协同工作的模式。vibe coding 更像是丢给 AI 一句非常模糊的话，例如“给我做个类似 Amazon 的网站”。而 TDD 和 SDD 不同，它们要求先拆分任务，把任务拆得尽可能细，避免 AI 的上下文窗口爆掉并产生严重幻觉。要把任务拆得足够细，需要架构师对微服务架构、系统边界和代码耦合位置足够熟悉，而不是盲目让 AI 胡作。这才是使用 AI 辅助编程的核心所在。如果写了 spec 之后，每次生成的函数名称、类名称、变量名称都不同，并且造成严重困扰，这通常说明 spec 本身写得有问题，不够详细。spec 是让 AI 去做细枝末节、需要大量人力和重复劳动的地方，而不是让 AI 自己设计，也不是让 AI 自己决定这里应该用 KISS 还是 DRY。架构师一定要在开始的时候就对整个系统有充分了解，否则做出来的系统一定会是歪的。AI 辅助编程已经是大势所趋，以后重复性工作必然被替代。同时，SDD 和 TDD 会极度烧 token，因为一切以 spec 为主，spec 变了，代码就要大规模删改，至少在大版本上会完全删改；老代码也需要被蒸馏成可复用的 skill 或 spec，这本身也是极度燃烧 token 的过程。以 spec 为主，曾经是工程师极度痛恨的事情，因为 spec 往往是完全过时的文档，里面版本丛生，如果不是为了硬着头皮发布，几乎完全没法看。但在 AI 辅助工程里，现在的思维模式必须颠倒过来，否则就会被时代淘汰。工程师要从单纯干活的人，转变成更接近 PM、架构师和规格负责人的角色。这是一个极难的转变。这不是大脑外包，而是必须真正理解工程、真正做过工程，才能达成的事情。它反而要求更纯粹、更深度的思考，并且要把项目垮塌、系统失控、协作失败的经验教训不断总结成更好的 spec 和工作流。所以最重要的事情变成了：你使用的每一个技术栈、每一个实现细节，你都必须先懂，AI 才不容易做错。AI 协作会变成一场永无止境、考验大脑极限承压能力的无边战争。spec 也必须是自己先看懂、先深度理解、先知道所有问题的东西，而不是随手丢给 AI 的文本。这要求真正的架构能力：知道什么轮子不应该自己造，也知道有哪些成熟的轮子可以用。也许有一天 AI 会完全取代人类，连架构都能自己做出来，但这需要极限上下文能力和极限的大海捞针能力。依靠目前的 AI 似乎还做不到。即使未来能做到，也会非常昂贵。",
        aiArchitectTokenCaption:
...
function renderPersonal(lang) {
  ...
  const pianoGroups = page.pianoGroups
    ...
    .join("");
  const catImages = page.catImages
    .map(
      (image) => `
        <figure class="project-image">
          <button class="project-image-button" type="button" data-lightbox-src="${escapeHtml(
            image.src,
          )}" data-lightbox-caption="${escapeHtml(image.caption)}" aria-label="${escapeHtml(image.caption)}">
            <img src="${escapeHtml(image.src)}" alt="${escapeHtml(image.caption)}" loading="lazy" />
          </button>
          <figcaption>${escapeHtml(image.caption)}</figcaption>
        </figure>
  ...
  host.innerHTML = `
    ...
    <section id="cat">
      ...
      <div class="home-panel">
        ...
        <div class="project-image-grid">${catImages}</div>
      </div>
    </section>
    <section id="piano">
