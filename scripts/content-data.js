  ...
  projects: {
    catalog: {
      ...
      "billzi2016.github.io": {
        ...
        language: "HTML",
        noteEn: "The repository for this bilingual personal website. It combines academic-profile presentation, project indexing, publications, theme and language switching, interactive visuals, and a local MIDI player, making it both a portfolio artifact and a front-end engineering exercise.",
        noteZh: "这是当前这个双语个人网站本身的仓库，里面整合了学术主页、项目索引、论文展示、主题与语言切换、交互式背景以及本地 MIDI 播放器。它既是我的求职作品集载体，也是一次完整的前端工程与信息架构整理实践。",
      },
      "Study": {
        name: "Study",
        url: "https://github.com/billzi2016/Study",
        language: "none",
        noteEn: "A general-purpose study repository that acts as a container for notes, experiments, and smaller learning traces. Repositories like this are useful because they preserve the breadth of work that does not yet justify a dedicated public project but still informs later engineering decisions.",
        noteZh: "这是一个通用型学习仓库，用来承载笔记、实验和一些还不足以单独拆成公开项目的小型探索。这类仓库虽然不一定像主项目那样完整，但能保留很多后来会进入正式工程决策的学习痕迹和技术积累。",
      },
      "vlm-hybrid-gallery": {
        name: "vlm-hybrid-gallery",
        url: "https://github.com/billzi2016/vlm-hybrid-gallery",
        language: "Python",
        noteEn: "A hybrid VLM gallery project combining image understanding, embeddings, indexing, and a web-facing interface. It represents my interest in building end-to-end multimodal systems instead of isolating the model from the retrieval and presentation layers.",
        noteZh: "这是一个混合型 VLM 展示项目，把图片理解、embedding、索引和网页界面组合在一起。它体现了我对端到端多模态系统的兴趣，不是只停留在模型本身，而是把检索层、表示层和展示层一起打通。",
      },
      "ATmega2560-Graphical-RPN-Scientific-Calculator": {
        ...
        language: "C++",
        noteEn: "A graphical RPN scientific calculator built around ATmega2560 with dual-display design, matrix keyboard input, and persistent storage ideas. It is a strong embedded-systems project because it mixes interface design, hardware constraints, input ergonomics, and real device-oriented software architecture.",
        noteZh: "这是一个基于 ATmega2560 的图形化 RPN 科学计算器项目，采用双显示结构、矩阵键盘输入和持久化存储设计。它很能体现我的嵌入式能力，因为它把界面设计、硬件约束、输入效率和面向真机的软件架构放到了一起。",
      },
      "ATMega328p-RPN-Scientific-Calculator": {
        ...
        language: "C++",
        noteEn: "A more compact embedded scientific calculator project around Arduino-class hardware and RPN input. Compared with the graphical version, it emphasizes how to build a maintainable calculator stack within tighter hardware limits and a simpler display environment.",
        noteZh: "这是一个更轻量的嵌入式 RPN 科学计算器项目，围绕 Arduino 级别硬件展开。相比图形版，它更强调如何在更紧的资源限制和更简单的显示环境下，仍然做出结构清晰、可维护的计算器系统。",
      },
      "ATmega2560-LCD12864-Game-Of-Life": {
        ...
        language: "C++",
        noteEn: "An embedded Conway's Game of Life implementation on ATmega2560 with LCD12864, keypad control, and save/load support. It is a small but concrete example of low-level display driving, input handling, and stateful application design on resource-constrained hardware.",
        noteZh: "这是一个运行在 ATmega2560 上的生命游戏项目，配合 LCD12864、键盘控制以及存读档功能。虽然体量不算大，但很能体现我在资源受限硬件上做显示驱动、输入处理和状态型应用设计的实际能力。",
      },
      "Homemade-Tiny-OS": {
        name: "Homemade-Tiny-OS",
        url: "https://github.com/billzi2016/Homemade-Tiny-OS",
        language: "Python",
        noteEn: "A user-space tiny operating-system simulator with virtual disks, file-system structures, process scheduling, and shell behavior. I built it as a systems-learning project that focuses on state, persistence, boundaries, and testability rather than pretending to be a toy kernel screenshot demo.",
        noteZh: "这是一个运行在用户态的微型操作系统仿真器，包含虚拟磁盘、文件系统、进程调度和 shell 等机制。我做它不是为了“像内核截图那样演示一下”，而是为了认真练系统里的状态、持久化、模块边界和可测试性问题。",
      },
      "bignum-from-scratch": {
        name: "bignum-from-scratch",
        url: "https://github.com/billzi2016/bignum-from-scratch",
        language: "Python",
        noteEn: "A high-precision arithmetic library that implements BigInt and BigFloat from scratch without leaning on Python's higher-level precision helpers. It is a compact but honest exercise in numerical representation, carry logic, precision control, and API design for mathematical software.",
        noteZh: "这是一个从零实现高精度大数运算的仓库，包含 BigInt 与 BigFloat，并刻意不依赖更高层的大数工具。它虽然规模不算最大，但很扎实地覆盖了数值表示、进位逻辑、精度控制和数学软件接口设计这些基础能力。",
      },
      "Homemade-CPython": {
        name: "Homemade-CPython",
        url: "https://github.com/billzi2016/Homemade-CPython",
        language: "Python",
        noteEn: "A long-horizon project for reconstructing the core path from Python source to bytecode and simplified VM execution. The emphasis is on building a stable, testable educational compiler/runtime path instead of making a one-shot fake interpreter demo.",
        noteZh: "这是一个长期项目，目标是重建从 Python 源码到 `.pyc` 和简化虚拟机执行的核心链路。它的重点在于搭出稳定、可测试、可持续扩展的编译器 / 运行时学习路径，而不是做一个一次性的“假解释器”演示。",
      },
      ...
      "Visual-Basic-Core-Compiler": {
        ...
        language: "Python",
        noteEn: "A Python implementation of a core Visual Basic compiler pipeline that goes from source through tokens, AST, semantics, IR, and portable C. I value this repository because it shows I can reason about language design and compilers beyond the usual ML-centric project space.",
        noteZh: "这是一个用 Python 实现的 Visual Basic 核心编译器项目，打通了从源码、Token、AST、语义分析到 IR 和可移植 C 的完整链路。它能很好地说明我不只做 AI，也能系统地做语言设计和编译器这类偏底层的软件工程问题。",
      },
      "Self-Hosting-C-Core-Compiler": {
        ...
        language: "C",
        noteEn: "A self-hosting C-subset compiler project whose ambition is to bootstrap itself and eventually step away from the original Python implementation. It is one of the stronger software-engineering projects in the portfolio because it mixes parsing, code generation, bootstrapping, and long-term architecture discipline.",
        noteZh: "这是一个以自举为目标的 C 子集编译器项目，希望最终能够用自己编译自己的源码，逐步摆脱最初的 Python 版本。它是我作品集中偏软件工程、偏底层的强项目之一，因为其中同时涉及解析、代码生成、自举链和长期架构演进。",
      },
      ...
      "RS485-Modbus-Concrete-Sensor-Monitor": {
        ...
        language: "Python",
        noteEn: "A real-time concrete-sensor monitoring system built with Django, Django Ninja, and Redis around RS485/Modbus flows. It is a practical industrial-style sensing project that connects field protocols, backend services, and live monitoring rather than stopping at a data-collection script.",
        noteZh: "这是一个基于 Django、Django Ninja 和 Redis 的混凝土传感器实时监测系统，围绕 RS485 / Modbus 流程展开。它比较像真正工业场景里的 sensing project，把现场协议、后端服务和实时监控连在了一起，而不只是一个采集脚本。",
      },
      ...
      "SQLite-Chaos-Tester": {
        ...
        language: "Python",
        noteEn: "A destructive and stress-oriented testing framework for SQLite under concurrency, corruption, abnormal termination, and resource pressure. This is a strong systems repository because it demonstrates curiosity about failure modes, consistency, recovery, and engineering under hostile conditions.",
        noteZh: "这是一个面向 SQLite 极端场景的压测与破坏性测试框架，覆盖并发、损坏、异常终止和资源耗尽等问题。它是一个很强的系统工程项目，因为它体现了我对失败模式、一致性、恢复能力以及恶劣条件下软件行为的真实兴趣。",
      },
      "speckit": {
        name: "speckit",
      ...
      "MIMO-FMCW-Radar-Simulator-Multiprocess": {
        ...
        language: "Python",
        noteEn: "A Python simulator for mesh-based MIMO FMCW radar scenes with visibility filtering and multiprocessing acceleration. It is closely aligned with my mmWave sensing background and demonstrates how I approach radar problems from both signal-processing and software-systems angles.",
        noteZh: "这是一个面向 MIMO FMCW 雷达场景的 Python 模拟器，支持基于网格模型的可见面筛选和多进程加速。它和我的毫米波感知背景高度相关，也很能体现我会同时从信号处理和软件系统两个角度去做雷达问题。",
      },
      "automl-autodl": {
        name: "automl-autodl",
