    ...
    personal: {
      ...
      zh: {
        ...
        aiArchitectParagraphs: [
          ...
          "这项工作并不是为了盲目生成页面或代码。我把 Codex / Claude Code 放在 human-in-the-loop 的工作流里使用，并通过 spec-first、review-driven 的方式推进，同时持续引入 TDD、SDD 和 CI/CD 等工程实践，去提升可读性、多平台支持能力、可维护性、后续维护效率、安全性和稳定性。",
          ...
          "架构仍然必须由人来设计和搭建。AI 是 worker，是加速器，但不是系统判断权的拥有者。它可以显著加快重复实现、结构整理、文档补全和各种苦力劳动，但真正的思考必须始终由人来完成。人必须始终知道自己在做什么、为什么这样做、系统整体应当如何成立；如果这一点已经丢失，那么这个项目往往就不是继续盲修盲补的问题，而是应该整体删除后重新设计。",
