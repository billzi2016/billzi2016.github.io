...
这个仓库是个人主页，也是公开的工程样本。代码要保持干净、可预测、速度快，并且方便其他工程师或 HR 技术面相关人员阅读。
...
| 音乐页/播放器 | `src/components/MusicInitialContent.astro`、`src/scripts/site/site-music.js`、`src/styles/site/music.css` | 必须保持页面切换时音乐不断。 |
| 搜索 | `src/scripts/site/site-search.js`、`src/styles/site/search.css` | 搜索索引保持本地、浏览器侧运行。 |
| Publications | `src/components/PublicationsInitialContent.astro`、`src/scripts/site/site-citation.js` | 引用复制按钮依赖 `.pub-copy-btn`。 |
