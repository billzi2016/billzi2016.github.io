...
持久音乐播放是运行时契约，不是页面局部 widget。`site-music.js` 负责曲目 index、音量、播放位置、播放状态、播放列表控制和 audio 元素放置。导航代码在替换页面内容前必须保存播放状态；任何音乐 UI 都必须复用或恢复已有 audio 元素，不要创建新的独立播放器。浮动播放器和完整音乐页只是同一套播放系统的两个视图。
